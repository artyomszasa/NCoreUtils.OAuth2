# **********************************************************************************************************************
# BUILD IMAGE
FROM mcr.microsoft.com/dotnet/core/sdk:3.1.201-bionic-arm64v8 AS build-env-oauth2
# PREFETCH FSharp.Core and System.Net.Http INTO SEPARATE LAYER
COPY ./NuGet.Config /tmp/prefetch/
RUN mkdir -p /tmp/prefetch \
    && cd /tmp/prefetch \
    && dotnet new web \
    && dotnet add package NCoreUtils.IO --version 3.1.0-alpha0 --no-restore \
    && dotnet add package NCoreUtils.Data.Protocol --version 3.1.0-alpha0 --no-restore \
    ...
    && dotnet add package System.Threading.Tasks.Extensions --version 4.5.4 --no-restore \
    ...
    && dotnet add package NCoreUtils.Data.EntityFrameworkCore --version 3.1.0-alpha0 --no-restore \
    && dotnet add package Antlr4.Runtime --version 4.6.6 --no-restore \
    && dotnet restore -r ubuntu.18.04-arm64 -v n
WORKDIR /app
# COPY NUGET config
COPY ./NuGet.Config ./
# COPY PROJECT FILES
...
# RESTORE PACKAGES
RUN dotnet restore ./NCoreUtils.OAuth2.WebService/NCoreUtils.OAuth2.WebService.fsproj -r ubuntu.18.04-arm64 -v n
# COPY SOURCES
...
# PUBLISH APPLICATION
RUN dotnet publish ./NCoreUtils.OAuth2.WebService/NCoreUtils.OAuth2.WebService.fsproj -c Release -r ubuntu.18.04-arm64 --no-restore -o /app/out

# **********************************************************************************************************************
# RUNTIME IMAGE
FROM mcr.microsoft.com/dotnet/core/runtime-deps:3.1.3-bionic-arm64v8
WORKDIR /app
ENV DOTNET_ENVIRONMENT=Production \
    ASPNETCORE_ENVIRONMENT=Production \
    ASPNETCORE_LISTEN_AT=0.0.0.0:80
# COPY APP
