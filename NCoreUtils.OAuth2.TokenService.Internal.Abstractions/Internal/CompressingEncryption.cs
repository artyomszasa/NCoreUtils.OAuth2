...
public class CompressingEncryption(ITokenEncryption encryption) : ITokenEncryption
{
    private const int MinBufferSize = 8 * 1024;

    private const int MaxBufferSize = 8 * 8 * 1024;

    private readonly ITokenEncryption _encryption = encryption ?? throw new ArgumentNullException(nameof(encryption));
