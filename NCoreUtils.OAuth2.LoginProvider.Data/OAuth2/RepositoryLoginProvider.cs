...
{
    protected ILoginProviderConfiguration Configuration { get; } = configuration ?? throw new ArgumentNullException(nameof(configuration));

    protected IDataRepository<TUser> UserRepository { get; } = userRepository ?? throw new ArgumentNullException(nameof(userRepository));
...
#if NET7_0
    [UnconditionalSuppressMessage("Trimming", "IL3050", Justification = "Should be handled by the underlying LINQ provider.")]
    [UnconditionalSuppressMessage("Trimming", "IL2026", Justification = "Should be handled by the underlying LINQ provider.")]
#endif
